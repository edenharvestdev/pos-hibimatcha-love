...
import { TopActionBar, EmptyState, useApp } from "@/components";
...
export const PagePnLDashboard = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-muted/20">
      <TopActionBar title={t("P&L Dashboard", "แดชบอร์ด P&L")} />
      ...
      <div className="p-6 flex-1 overflow-y-auto space-y-6 max-w-6xl mx-auto w-full">
        <div className="flex items-center gap-4 bg-card p-4 rounded-xl border">
           <div className="flex items-center gap-2">
             <label className="text-sm font-medium">{t("From", "ตั้งแต่")}</label>
             <input 
               type="date" 
           ...
           <div className="flex items-center gap-2">
             <label className="text-sm font-medium">{t("To", "ถึง")}</label>
             <input 
               type="date" 
               className="input max-w-[160px]"
               value={dateRange.to}
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
               <div className="bg-card p-5 rounded-xl border shadow-sm">
                 <p className="text-sm text-muted-foreground font-medium">{t("Revenue", "ยอดขาย (Revenue)")}</p>
                 <h3 className="text-2xl font-bold text-primary mt-1">฿{pnl?.revenue?.toLocaleString()}</h3>
                 <p className="text-xs text-muted-foreground mt-2">{pnl?.orderCount} {t("orders", "ออเดอร์")}</p>
               </div>
               <div className="bg-card p-5 rounded-xl border shadow-sm">
                 <p className="text-sm text-muted-foreground font-medium">{t("COGS", "ต้นทุนขาย (COGS)")}</p>
                 <h3 className="text-2xl font-bold text-destructive mt-1">฿{pnl?.cogsCost?.toLocaleString()}</h3>
                 <p className="text-xs text-muted-foreground mt-2">
                   {pnl?.revenue ? Math.round((pnl.cogsCost / pnl.revenue) * 100) : 0}% {t("of revenue", "ของยอดขาย")}
                 </p>
               </div>
               <div className="bg-card p-5 rounded-xl border shadow-sm">
                 <p className="text-sm text-muted-foreground font-medium">{t("Expenses", "ค่าใช้จ่าย (Expenses)")}</p>
               ...
               <div className="bg-card p-5 rounded-xl border shadow-sm relative overflow-hidden">
                 ...
                 <div className="relative">
                   <p className="text-sm text-primary/80 font-medium">{t("Net Profit", "กำไรสุทธิ (Net Profit)")}</p>
                   ...
                   <div className="inline-block mt-2 px-2 py-0.5 bg-primary/10 text-primary rounded text-xs font-semibold">
                     {t("Margin", "อัตรากำไร")} {pnl?.margin}%
                   </div>
                 </div>
               </div>
            </div>

            <h3 className="text-lg font-bold mt-8 mb-4">{t("Compare Branches", "เปรียบเทียบสาขา")}</h3>
            <div className="bg-card border rounded-xl overflow-hidden shadow-sm">
              <table className="w-full text-sm text-left">
                <thead className="bg-muted/50 border-b">
                  <tr>
                    <th className="p-3 font-semibold">{t("Branch", "สาขา")}</th>
                    <th className="p-3 font-semibold text-right">{t("Revenue", "ยอดขาย")}</th>
                    <th className="p-3 font-semibold text-right">{t("Expenses", "ค่าใช้จ่าย")}</th>
                    <th className="p-3 font-semibold text-right">{t("Royalty Fee", "ค่า Royalty")}</th>
                    <th className="p-3 font-semibold text-right">{t("Net Profit", "กำไรสุทธิ")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  ...
                  {multiBranch?.length === 0 && (
                    <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">{t("No data for this period", "ไม่มีข้อมูลในช่วงเวลานี้")}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
